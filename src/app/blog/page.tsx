...
import { getBlogs, getCategories, getFeaturedBlog } from "@/lib/queries/blog";

export const revalidate = 60;

export const metadata: Metadata = {
  title: "HexaKode Blog | Engineering & Product Design Insights",
  description: "Read the latest articles on web engineering, mobile apps, UI/UX design, technology consulting, and software development insights from the HexaKode team.",
  alternates: {
    canonical: "https://www.hexakode.in/blog",
  },
};
...
export default async function BlogIndexPage(props: PageProps) {
  ...
  const [categories, featuredPost, blogsData] = await Promise.all([
    getCategories(),
    getFeaturedBlog(),
    getBlogs({ search, category, tag, sort, page, limit }),
  ]);
          ...
          <Container className="flex flex-col gap-16">
            ...
            <div className="w-full">
              <Suspense fallback={
                ...
                <BlogGrid posts={displayPosts} total={showFeatured ? total - 1 : total} limit={limit} />
              </Suspense>
            </div>
